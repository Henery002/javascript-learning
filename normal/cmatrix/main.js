...
Matrix.prototype.change = function (id, opacity) {
...
var matrixManager = (function () {
  ...
  var matrixAdd = function (id, type, text) {
    ...
    dom.onclick = function () {
      matrixFactory.get(type).change(id, 0);
    };
  ...
  var setExternalState = function (obj, id) {
    ...
    for(var j = 0; j < matrixDatabase[rangerIndex].length; j++){
      if (matrixDatabase[i][j].id == id) {
        console.log(id);
        Object.keys(matrixDatabase[i][j]).map(function (attr) {
